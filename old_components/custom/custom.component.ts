...
export class CustomComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
      if (!user) {
        ...
        firebase
          .database()
          .ref("Users/" + user.uid + "/ig")
          .on("value", function (igData) {
            if (igData.val()) {
              const igForm = document.getElementById(
                "defaultContactFormig"
              ) as HTMLInputElement;
  ...
  sendEmail(data) {
    ...
    firebase.auth().onAuthStateChanged(async function (user) {
      if (user) {
        if (
          self.attachments.length === self.numToUpload &&
          self.numToUpload === self.imgur.uploadedImgs.length
        ) {
          ...
          sendEmail().then((res) => {
            Email.send({
              ...
              if (message === "OK") {
                ...
                self.router.navigate(["/login/open-orders"], {
          ...
          alert(
            "Try again in a few seconds as the files upload. If this still does not work, refresh the page"
          );
        }
      } else {
        self.askLogin();
  ...
  getDesiredService(event) {
    this.selectedService = (document.getElementById(
      "serviceSelect"
  ...
  onSubmit(event) {
    ...
    if (
      event.printOpt != "No Print" &&
      (!event.size || (event.size === "Other" && !event.otherSize))
    ) {
      this.sizeErr = true;
    } else {
      this.sizeErr = false;
  ...
  getDesiredSize(event) {
    this.selectedSize = (document.getElementById(
      "sizeSelect"
    ) as HTMLInputElement).value;
  }
  ...
  addCustomToDB(user, data) {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
      if (user) {
        ...
        data.uid = user.uid;
        updates[
          "Users/" + user.uid + "/Open-orders-custom/" + `${time}`
        ] = data;
        updates[
          "Admin/" + "Open-orders-custom/" + user.uid + "/" + `${time}`
        ] = data;
        firebase.database().ref().update(updates);
      }
    });
  ...
  getPrices() {
    ...
    firebase
      .database()
      .ref("Products/Custom/")
      .on("value", function (customPrices) {
