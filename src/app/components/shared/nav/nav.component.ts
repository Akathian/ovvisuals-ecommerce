...
export class NavComponent implements OnInit {
  ...
  onLogin() {
    firebase.auth().onAuthStateChanged(function (user) {
      ...
      if (user) {
        if (user.photoURL) {
          document.getElementById('login').innerHTML = `<img id='loginImg' src='${user.photoURL}' width='26px'/> `
          document.getElementById('loginImg').style.borderRadius = '100%'
        } else {
          document.getElementById('logoutImg').style.fill = 'green'
        }
        firebase.database().ref('Users/' + user.uid + '/Cart/' + 'itemQty').on('value', function (itemQtyData) {
          if (itemQtyData) {
            let currQty = itemQtyData.val()
