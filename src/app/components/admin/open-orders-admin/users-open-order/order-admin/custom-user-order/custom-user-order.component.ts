...
export class CustomUserOrderComponent implements OnInit {
  ...
  moveToIntermediate() {
    let self = this
    let up = (this.cat.charAt(0).toUpperCase() + this.cat.slice(1)).replace("_", "-")
  ...
  sendEmail(data) {
    let subject = "OVVisuals Request Update"
    let title = `Hello ${data.name}`
    let subHead = `Regarding your recent order with me, I've provided a quote with how much your request will cost. Have a look and reply to this email if you want to change anything!`
    let self = this
