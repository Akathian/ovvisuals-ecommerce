...
export class UserOrderComponent implements OnInit {
  ...
  parseOrder(order) {
    ...
    const orderObj = Object.values(order[1]);
    if (this.uid === 'all') {
  ...
  getPromiseDate(time, ship) {
    ...
    switch (ship) {
      ...
      case '3': {
        const d3Min = this.addWorkDays(date, 6, null);
        const d3Max = this.addWorkDays(date, 10, null);
        this.promiseDate = d3Min.toDateString() + ' - ' + d3Max.toDateString();
        break;
      }
      case '4': {
        const d4Min = this.addWorkDays(date, 4, null);
        const d4Max = this.addWorkDays(date, 6, null);
