...
export class ShippingComponent implements AfterViewChecked, OnInit {
  ...
  calculateShip() {
    ...
    const d2 = this.addWorkDays(this.d, 12, 6);
    ...
    const d3Min = this.addWorkDays(this.d, 6, null);
    const d3Max = this.addWorkDays(this.d, 10, null);
    document.getElementById('standard').innerText = d3Min.toDateString() + ' - ' + d3Max.toDateString();

    const d4Min = this.addWorkDays(this.d, 4, null);
    const d4Max = this.addWorkDays(this.d, 6, null);
